                    ...
                    <div class="card mb-4 card-primary shadow">
                        ...
                        <div class="card-body">
                            <div class="row justify-content-center">
                                <div class="col-12">
                                    <form id="formManageHome">
                                        <input type="hidden" name="domain_id" id="domain_id" value="{{$domain_id}}">
                                        <div class="row">
                                            <div class="col-sm-12 col-md-12 col-lg-12">
                                                <div class="form-group">
                                                    <label>Direct Link</label>
                                                    <input class="form-control" type="text" name="direct_link" value="{{ $domain->direct_link }}" />
                                                </div>
                                            </div>
                                            @php
                                                $home = [
                                                ...
                                                $manage_home = json_decode($domain->manage_home, true) ?? [];
                                            @endphp
                                            @foreach($home as $key => $label)
                                                <div class="col-sm-6 col-md-6 col-lg-6">
                                                    <div class="form-group">
                                                        <label>{{ $label }}</label>
                                                        <textarea class="form-control" type="text" rows="5" name="manage_home[{{ $key }}]" id="{{ $key }}" >{{ $manage_home[$key] ?? '' }}</textarea>
                                                    </div>
                                                </div>
                                            @endforeach
                                        </div>
...
@push('js')
    <script>
        $(document).ready(function () {
            ...
            const tableLoadDomain = setupDataTable(
                ...
                function (row, data) {
                    ...
                    $(row).on('click', function () {
                        var url = "{{ route('admin.domain.manage_home', ['id' => ':id']) }}";
            ...
            $('#formManageHome').on('submit',function (event){
                event.preventDefault();
                var formData = new FormData($("#formManageHome")[0]);
                var  url = "{{ route('admin.domain.updateHome') }}";
