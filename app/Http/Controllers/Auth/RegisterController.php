...
//    protected $redirectTo = '/user/dashboard';
    ...
    public function register(Request $request)
    {
        ...
        if ($request->ajax()) {
            return route('user.dashboard_user.home');
        }
    ...
    protected function registered(Request $request, $user)
    {
        ...
        $ul['user_id'] = $user->id;
        $ul['user_ip'] = getIp();
        $ul['longitude'] = $location['lat']?? null;
        $ul['latitude'] = $location['lon']?? null;
        $ul['location'] = $location['city']?? null;
        $ul['country_code'] = $location['iso_code']?? null;
        $ul['browser'] = $agent->platform();
        $ul['os'] = $agent->browser();
        $ul['country'] = $location['country']?? null;
        UserLogin::create($ul);
    }
